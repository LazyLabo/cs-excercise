...
# １．メッセージを表示する関数を使ってみる

> ブラウザに「hello!」とメッセージのポップアップを表示させるにはどうしたらいいでしょうか？
...
- ブラウザをスタートしたら自動的に作られる```window``` というオブジェクトに仕事をさせました
- ブラウザには他にもいくつかオブジェクトがあって、それぞれいろんな関数を持っています
- オブジェクトは、ほとんどの場合、関数だけでなくデータも持っています（今回は関数について勉強します）
- 文字の列（文字列）はシングルクオーテーションマークで囲みました

# ２．任意のページをロードする関数を使ってみる

> 今使っているブラウザのタブに他のウェブサイト（例：https://google.com）のページを表示させるにはどうしたらいいでしょうか？
...
- ブラウザに命令して、指定したウェブサイトのページをダウンロードして、その内容を表示するようにしました。
- カッコ（）の中に表示させたいウェブサイトのページを指定しました
- カッコ（）を使って関数に渡すものを引数と呼びます。英語ではparemeterとかargumentと呼びます
- ごく単純なプログラミングを行いました
...
> add という名前の関数を作ります。パラメーターは２つで、a と x をとります。関数が定義できたら、それを使って3と5を足した数求めましょう

### ヒント

関数の呼び出し方は alert() や open()と同様に、カッコにパラメーターを指定してやります
...
```var add = ``` の部分でaddという名前の関数を作ろうとしています
``` (a, x) ``` の部分で、この関数は２つのパラメターを取り、それぞれの名前を決めています（名前は何でもいいですが、意味がある場合は名前を工夫した方が読みやすいです）
``` => a + x ``` の部分で、この関数の実際の仕事内容を設定します。この場合はaとxを足す、という作業をしました。作業内容のことを「実装（implementation）」と呼びます
