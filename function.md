...
- １行目で、```multipl(2)(1)``` ではなく ```multipl(2)``` で止めていることに注目してください。
- これは a=2 で固定したけれど、x についてはまだ未知のままであることを表しています。戻って来た値は x を取る関数なので、```multiplyBy2```という名前の関数にして、５回呼び出しています。
