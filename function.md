...
このワークシートではプログラミングにおける**関数**（英語でfunction）を、ハンズオンスタイル（手で動かす形式）で学びます。

Gooogle Chrome ブラウザの開発者向けツールにある Console にJavaScriptのコードを入力して、気軽に学ぶことが出来ます（今この瞬間、このページを Chrome で閲覧中なら、F12キーを押して開発者向けツールの画面を開いて気軽にコードを試すことが出来ます） 

[実際に学んでいる様子を動画](https://youtu.be/ujweIiFze_s)にしました。参考にしてください。
