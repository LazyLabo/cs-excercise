...
ここまでは関数の引数を２つ以上ある時は、```()```の中に```,```で区切って渡して、関数を定義していました。このスタイルで定義した関数は ```f(2, 3)``` のように呼び出しました。

これとは別に引数をひとつずつ、```=>``` で区切って渡して関数を定義する方法もあります。このスタイルで定義した関数は ```f(2)(3)``` というやり方で呼び出します。このスタイルの関数はソフトウェア開発の現場でも重要度が上がってきています。

ここまで練習した関数をこの=>を使うスタイルに書き直してみましょう。
