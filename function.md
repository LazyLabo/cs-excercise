...
- ``` var multiply = ``` の部分は「代入」です（英語で assignment ）。これは数値を格納する変数と全く同じ書き方です。「= 記号の右側」を「= 記号の左側」に「入れる（代入している）」のです。記号の左側はオブジェクトでなくてはなりません。この場合は multiply という名前のオブジェクトを同時に定義しています（すでに定義されているなら ``` multiply = ```のように```var```を書きません。
- では、この場合の「= の右側」は``` (a, x) => a * x``` となっています。これを分解しましょう
- (a, x)の部分は関数の引数が何かを表しています。
- 「=> 記号（オペレータとも呼ぶ）」は「関数の実装はこちら」という意味です。「実装（英語で implementation)」とは「仕事の具体的な内容です」。今回は「掛け算の式」を使って掛け算という仕事をさせました。もっと複雑な仕事をさせることも出来ますが、この場合は multiply という名前を付けているのでピッタリそれだけさせます。
- 式を評価した結果は関数から「返されます」。「return オペレータ」を使う方法もありますがそれは別の機会に解説します。
...
# 2.2.1 関数を作る練習をしよう（その１）

## 問題

関数の書き方が少し難しいので、なれるためにいくつかの簡単な関数を書きます
a と x を足す関数。名前は add
